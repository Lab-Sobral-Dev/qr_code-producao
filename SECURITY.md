...
Depois de aplicar esse SQL, crie os usuarios em Authentication no Supabase. Para liberar acesso ao histórico, adicione o `usuario_id` do administrador na tabela `app_administradores`. A leitura publica dos QR Codes continua funcionando somente por `id`, via `consultar_alcool_registro`.
